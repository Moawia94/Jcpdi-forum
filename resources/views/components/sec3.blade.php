<section class="bg-primary-800 w-full content-center py-24 md:py-40">
    <div class="gap-8 items-center py-8 px-4 mx-auto max-w-screen-xl xl:gap-16 sm:py-16 lg:px-6">
        <h1 class="text-center mb-4 text-4xl tracking-tight font-extrabold text-primary-200 ">
            {!!__('hero.sec-3-title') !!}
        </h1>
        <div class=" md:grid md:grid-cols-2 justify-center items-center py-12 text-white text-sm md:text-xl">
            <div class=" flex text-4xl items-center justify-center">
                <img src="svg/shap-4.svg" alt="" class="h-24 -ml-16 ">
                {!!__('hero.sec-3-content-1') !!}
            </div>
            <div class=" flex text-4xl items-center justify-center">
                <img src="svg/shap-4.svg" alt="" class="h-24 -ml-16 ">
                {!!__('hero.sec-3-content-2') !!}
            </div>

        </div>
        <div class=" md:grid md:grid-cols-3 justify-center items-center py-12 text-white text-sm md:text-xl">

            <div class="text-center flex text-4xl items-center justify-center">
                <img src="svg/shap-4.svg" alt="" class="h-16 -ml-8 ">
                {!!__('hero.sec-3-content-3') !!}
            </div>
            <div class="text-center flex text-4xl items-center justify-center">
                <img src="svg/shap-4.svg" alt="" class="h-16 -ml-8 ">
                {!!__('hero.sec-3-content-4') !!}
            </div>
            <div class="text-center flex text-4xl items-center justify-center">
                <img src="svg/shap-4.svg" alt="" class="h-16 -ml-8">
                {!!__('hero.sec-3-content-5') !!}
            </div>

        </div>
    </div>
</section>
